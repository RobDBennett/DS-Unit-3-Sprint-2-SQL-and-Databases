...
### Rob's Answers to Part 4:

- In the Northwind database, what is the type of relationship between the
  `Employee` and `Territory` tables?

* They don't have any direct relationship, however, they get a one to many relationship as they move through the EmployeeTerritory table. EmployeeTerritory holds the EmployeeID and TerritoryId items and acts as a junction between the two. Within the EmployeeTerritory table you'll find that there are 9 employees assigned to 49 territories, with employees holding multiple territories but no territory holding multiple employees.

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

* MongoDB is a non-relational database and is designed to house many non-like items. For things like FaceBook where you might be housing pictures, documents, and data as it relates to their users. Typically, these databases are reserved for 'bigdata' issues, and aren't directly appropriate to most business use-cases. A firmer SQL relational database is better for most cases as you know what is stored there and how it is arranged, making queries cleaner.
The only real use-case for a non-relational database is when ACID doesn't need to be strictly maintained as there are very few errors pushed back in Mongo regardless of what you upload (it doesn't check for unique or repeat values by default).

- What is "NewSQL", and what is it trying to achieve?

* NewSQL is SQL's approach to the scalability of non-relational databases. Large companies like Amazon or financial instutitions need the speed of a NoSQL database, but the ACID requirements of a traditional DB, and this product is geared towards those solutions without forcing those companies to buy exponentially more machines or develop proprietary software. 